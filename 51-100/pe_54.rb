pokers = [["8C","TS","KC","9H","4S","7D","2S","5D","3S","AC"],
...
def extract x
  ...
  x.each do |i|
    v.push i[0,1].tr("23456789TJQKA","A-Z")[0] - "A"[0].ord + 1
    h.push i[1,1]
  end

  if h.uniq.size == 1 && v.sort == [9,10,11,12,13]
    return [10]
  end
  
  if h.uniq.size == 1 && v.sort[4] - v.sort[0] == 4 && v.uniq.size == 5
    temp = Array.new(v)
    temp.delete v.sort[4]
    return [9, v.sort[4], temp]
  end
  ...
  v.each do |x|
    if v.count(x) == 3
      temp = Array.new(v)
      temp.delete x
      return [7, x, temp] if temp.uniq.size == 1
    end
  end
  
  if h.uniq.size == 1
    return [6, v.sort[4], v.sort[0,4]]
  end

  if (v.sort[4] - v.sort[0]) == 4 && v.uniq.size == 5
    return [5, v.sort[4], v.sort[0,4]]
  end
